...
def run_single_draw(
    ...
    for step in range(sim_cfg.n_steps):
        ...
        for cand in candidates:
            ...
            p_base = approval_probs.get(fips, 0.44)  # National median fallback

            # Apply interventions if enabled (firm-borne scenarios)
            delta_p = 0.0
            if tax_cfg.enabled:
            ...
            # Sample approval draw from Beta distribution
            ...
            if threshold is None:
                # Laissez-faire: built if draw > 0.5
                built = approval_draw > 0.5
            elif firm_borne:
                # Firm optimizes investment to push approval above threshold
                cost = firm_optimize(p_base, n, threshold, cfg)
                if cost is not None:
                    # Firm can afford it; still sample approval with adjusted p
                    built = approval_draw > threshold
                    if built:
                        cand_firm_cost = cost
                    else:
                        monthly_rejected += 1
                else:
                    # Infeasible: base prob too low even with max investment
                    monthly_infeasible += 1
            else:
                # Majority/supermajority: built if draw > threshold
                built = approval_draw > threshold
            ...
            if built:
                ...
                monthly_built += 1
            elif threshold is not None and not firm_borne:
                monthly_rejected += 1
