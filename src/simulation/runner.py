...
def load_approval_probs(path: str | Path | None = None) -> dict[str, float]:
    """Load county approval probabilities as a FIPS → prob dict.

    Parameters
    ----------
    path : Path to county_approval_probs.csv. Uses default if None.

    Returns
    -------
    Dict mapping FIPS code → calibrated approval probability.
    """
    p = Path(path) if path else _APPROVAL_PROBS_PATH
